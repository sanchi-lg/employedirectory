import Axios from 'axios'
import React, { Component } from 'react'
import axios from 'axios'


export class Employeedet extends Component {
    constructor(props) {
        super(props)
    
        this.state = {
            id: this.props.match.params.id,emp:{}
        }
    }

    componentDidMount(){
        axios.get(`http://localhost:9000/getempbyid/${this.state.id}`)
        .then(res=>{
            this.setState({emp:res.data})
        })
    }

    render() {
    
        
        return (
            <div>
                 <div className="edimg">
                            <img src="data:image/png;base64," height={40} width={40}/>
                        </div>
                        <div>
                         <div className="left">Name</div>
        <div className="right">{this.state.emp.ename}</div>
                        </div>
                        <div>
                         <div className="left">Email</div>
                         <div className="right">{this.state.emp.email}</div>
                        </div>
                        <div>
                         <div className="left">Position</div>
                         <div className="right">{this.state.emp.epos}</div>
                        </div>
                        <div>
                         <div className="left">Salary</div>
                         <div className="right">{this.state.emp.esalary}</div>
                        </div>
                        <div>
                         <div className="left">Age</div>
                         <div className="right">{this.state.emp.eage}</div>
                        </div>
                        <div>
                         <div className="left">Date of joining</div>
                         <div className="right">{this.state.emp.dateofjoining}</div>
                        </div>

                        
            </div>
        )
    }
}

export default Employeedet
